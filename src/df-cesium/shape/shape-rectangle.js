...
import { getShapeCenter, getCoordinateByCartesian } from "../utils/utils";

export class EntitiesRectangleOpation extends ShapeBase {
  constructor(arg) {
    super(arg);
    ...
    this.addLineEntitys = []; //多边形覆盖实体-线实体   - 这里面的没有添加到shapelist中
    this.editIndex = 0; // pointPosition 操作索引
    this.temporaryPoints = []; // 临时点
    this.temporaryLines = []; // 临时线
  ...
  // 选中当前shape
  selectCurrentEntity() {
    this.setCurrentEntityLineSelectColor(); // 设置多边形边框颜色
    this.shapeAddPoint(); // 添加覆盖点
    // this.setCurrentEntityShow(false) // 隐藏多边形
  }

  // 设置当前多边形变颜色为选中
  setCurrentEntityLineSelectColor() {
    this.currentEntity.polyline.material = Cesium.Color.DODGERBLUE.withAlpha(1); // 变颜色
  }

  // 取消选中当前shape
  cancelSelectCurrentEntity() {
    this.removeAddEntitys(); // 删除新增实体
    // this.setCurrentEntityShow(true) // 展示多边形
  ...
  updateSelectCurrentShape() {
    ...
    this.removeAddEntitys(); // 先删除新增的实体
    // this.delAddLinLines() // 清空临时线
  ...
  updatePolyonShapePosition(position) {
  ...
  mouseMove(task, params) {
    if (task && task.task.key === "ADD_ACTIVE_RECTANGLE") {
      ...
      if (shapePosition.length > 0) {
        const startPoint = shapePosition[0]; // 起始点
        const point = [position.longitude, position.latitude]; // 鼠标点击点
        const reactPoints = [startPoint];
        reactPoints.push([point[0], startPoint[1]]);
        reactPoints.push(point);
        reactPoints.push([startPoint[0], point[1]]);

        this.updatePolyonShapePosition(reactPoints);
      }
    }
  }
